...
/*   Updated: 2025/09/17 17:07:38 by arbaudou         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	if (ft_strncmp(line, "NO ", 3) == 0 && !map->path_north)
		map->path_north = dup_path(line);
	else if (ft_strncmp(line, "SO ", 3) == 0 && !map->path_south)
		map->path_south = dup_path(line);
	else if (ft_strncmp(line, "EA ", 3) == 0 && !map->path_east)
		map->path_east = dup_path(line);
	else if (ft_strncmp(line, "WE ", 3) == 0 && !map->path_west)
		map->path_west = dup_path(line);
	else if (ft_strncmp(line, "F ", 2) == 0 && map->floor_color == 0)
		map->floor_color = parse_color(line);
	else if (ft_strncmp(line, "C ", 2) == 0 && map->ceiling_color == 0)
